...
namespace Minigine
{
	namespace Graphics
	{
        ...
        : graphicsDevice(parentDevice), vertexBuffer(parentDevice, true), indexBuffer(parentDevice, true)
		{
			...
			this->vertices = vector<VertexPositionColor>(PointBatch::MaxBatchSize);
			this->elementCount = 0;
			this->technique = NULL;
		...
		void PointBatch::Begin() throw(...)
		{
            ...
            {
                this->technique = new Minigine::Graphics::EffectTechnique("attribute vec4 position; attribute vec4 color; uniform mat4 world; varying vec4 outcol; varying vec2 outtex; void main() { gl_Position = position * world; outcol = color / 255.0; gl_PointSize = 10.0; } ", "precision mediump float; varying vec4 outcol; void main() { gl_FragColor = outcol; } ");
            }
            
			this->alreadyDrawing = true;
		}
        
		void PointBatch::Draw(const Vector2F& position, const float& size, const Color& color)
		{
